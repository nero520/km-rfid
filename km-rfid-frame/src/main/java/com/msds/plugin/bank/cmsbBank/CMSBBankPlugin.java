package com.msds.plugin.bank.cmsbBank;
 ...
 * @version 3.0
 */
@Component("cmsbBankPlugin")
public class CMSBBankPlugin extends PaymentPlugin {

	/** logger */
	private static final Logger logger = Logger.getLogger(CMSBBankPlugin.class
			.getName());
	...
	public String getInstallUrl() {
		return "cmsbBank/install.html";
	}

	@Override
	public String getUninstallUrl() {
		return "cmsbBank/uninstall.html";
	}

	@Override
	public String getSettingUrl() {
		return "cmsbBank/setting.html";
	}
	...
	public Map<String, Object> getParameterMap(String sn, BigDecimal amount,
		...
		String subject = pluginConfig.getAttribute("subject");
		;// 商品分类 "1092001"
		String currencytype = "01";// 币种,01人民币
		String autojump = "1";// 银行付款成功后是否自动跳转到取货页面：0→不跳转；1→跳转
		String waittime = "0";// 跳转到取货页面的等待时间，以秒为单位，默认5秒
		...
		String merbank = "CMSB";// 商户端所选支付银行当bankInput为0时，此字段只能为empty,在宝易互通上选择支付银行
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod,
		...
		// && payment.getAmount().setScale(2).compareTo(new
		// BigDecimal(amountsum).setScale(2))==0 && "01".equals(currencytype)
		// && "1".equals(state) && "2.00".equals(interFace) &&
		// checkMac.equals(mac)) {
		// flag=true;
		// }
		if (checkMac != null && mac != null
				&& checkMac.toLowerCase().equals(mac.toLowerCase())
				&& state.equals("1")) {
			flag = true;
		}

		// logger.warning("民生银行保卫互通数据：" + mac + "," + merchantid.toString() +
		// ","
		// + merorderid.toString() + "," + amountsum + "," + currencytype
		// + "," + state.toString() + "," + interFace);
		// logger.warning("民生银行数据：" + checkMac + ","
