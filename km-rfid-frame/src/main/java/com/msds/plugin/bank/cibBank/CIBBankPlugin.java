package com.msds.plugin.bank.cibBank;
 ...
 * @version 3.0
 */
@Component("cibBankPlugin")
public class CIBBankPlugin extends PaymentPlugin {

	/** logger */
	private static final Logger logger = Logger.getLogger(CIBBankPlugin.class
			.getName());
	...
	public String getInstallUrl() {
		return "cibBank/install.html";
	}

	@Override
	public String getUninstallUrl() {
		return "cibBank/uninstall.html";
	}

	@Override
	public String getSettingUrl() {
		return "cibBank/setting.html";
	}
	...
	public Map<String, Object> getParameterMap(String sn, BigDecimal amount,
		...
		String merbank = "CIB";// 商户端所选支付银行当bankInput为0时，此字段只能为empty,在宝易互通上选择支付银行
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod,
		...
		logger.warning("兴业银行网关支付保卫互通数据：" + mac + "," + merchantid.toString()
				+ "," + merorderid.toString() + "," + amountsum + ","
				+ currencytype + "," + state.toString() + "," + interFace);
		logger.warning("兴业银行网关支付数据：" + checkMac + ","
