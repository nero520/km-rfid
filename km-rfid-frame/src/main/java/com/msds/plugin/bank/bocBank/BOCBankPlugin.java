package com.msds.plugin.bank.bocBank;
 ...
 * @version 3.0
 */
@Component("bocBankPlugin")
public class BOCBankPlugin extends PaymentPlugin {

	/** logger */
	private static final Logger logger = Logger.getLogger(BOCBankPlugin.class
			.getName());
	...
	public String getInstallUrl() {
		return "bocBank/install.html";
	}

	@Override
	public String getUninstallUrl() {
		return "bocBank/uninstall.html";
	}

	@Override
	public String getSettingUrl() {
		return "bocBank/setting.html";
	}
	...
	public Map<String, Object> getParameterMap(String sn, BigDecimal amount,
		...
		String merbank = "BOC";// 商户端所选支付银行当bankInput为0时，此字段只能为empty,在宝易互通上选择支付银行
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod,
		...
		logger.warning("交通银行网关支付保卫互通数据：" + mac + "," + merchantid.toString()
				+ "," + merorderid.toString() + "," + amountsum + ","
				+ currencytype + "," + state.toString() + "," + interFace);
		logger.warning("交通银行网关支付数据：" + checkMac + ","
