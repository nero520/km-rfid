package com.msds.plugin.bank.heyihangpay;
 ...
 * @version 3.0
 */
@Component("heyihangpayPlugin")
public class HeyihangpayPlugin extends PaymentPlugin {

	/** logger */
	private static final Logger logger = Logger
			.getLogger(HeyihangpayPlugin.class.getName());
	...
	public String getInstallUrl() {
		return "heyihangpay/install.html";
	}

	@Override
	public String getUninstallUrl() {
		return "heyihangpay/uninstall.html";
	}

	@Override
	public String getSettingUrl() {
		return "heyihangpay/setting.html";
	}

	@Override
	public String getRequestUrl() {
		return "https://www.umbpay.com/hpay/unSecurity/getMerchantInfo.do";
	}
	...
	public Map<String, Object> getParameterMap(String sn, BigDecimal amount,
		...
		String merbank = "empty";// 商户端所选支付银行当bankInput为0时，此字段只能为empty,在宝易互通上选择支付银行
		String tradetype = "0";// 交易类型： 0→即时到账；1→担保交易
		String bankinput = "0";// 0→其他；1→在商户端选择银行
		String interFace = "M1.00";// 接口版本号，默认填写M1.00
		String userid = "";
		String paytype = "00";// 账户支付类型
		String orderdate = new SimpleDateFormat("yyyyMMddHHmmss")
				.format(new Date());
		...
		signMap.put("orderdate", orderdate);
		signMap.put("pdtname", "");
		signMap.put("remark", "");
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod,
		...
		String payorderid = request.getParameter("payorderid"); // 流水
		String orderdate = request.getParameter("orderdate");// 订单日期
		String interFace = request.getParameter("interface");// 接口版本
		...
		parameterMap.put("payorderid", payorderid);
		...
		// && "1".equals(state) && "M1.00".equals(interFace) &&
		// checkMac.equals(mac)) {
		// flag=true;
		// }
		...
		logger.warning("合行通支付保卫互通数据：" + mac + "," + merchantid.toString() + ","
				+ merorderid.toString() + "," + amountsum + "," + currencytype
				+ "," + state.toString() + "," + interFace);
		logger.warning("合行通支付数据：" + checkMac + ","
	...
	public String getPaymentSerialNumber(HttpServletRequest request) {
		return request.getParameter("payorderid");
	}
}