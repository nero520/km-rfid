package com.msds.plugin.bank.szdbBank;
 ...
 * @version 3.0
 */
@Component("szdbBankPlugin")
public class SZDBBankPlugin extends PaymentPlugin {

	/** logger */
	private static final Logger logger = Logger.getLogger(SZDBBankPlugin.class
			.getName());
	...
	public String getInstallUrl() {
		return "szdbBank/install.html";
	}

	@Override
	public String getUninstallUrl() {
		return "szdbBank/uninstall.html";
	}

	@Override
	public String getSettingUrl() {
		return "szdbBank/setting.html";
	}
	...
	public Map<String, Object> getParameterMap(String sn, BigDecimal amount,
		...
		String merbank = "SZDB";// 商户端所选支付银行当bankInput为0时，此字段只能为empty,在宝易互通上选择支付银行
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod,
		...
		logger.warning("深圳发展银行网关支付保卫互通数据：" + mac + "," + merchantid.toString()
				+ "," + merorderid.toString() + "," + amountsum + ","
				+ currencytype + "," + state.toString() + "," + interFace);
		logger.warning("深圳发展银行网关支付数据：" + checkMac + ","
