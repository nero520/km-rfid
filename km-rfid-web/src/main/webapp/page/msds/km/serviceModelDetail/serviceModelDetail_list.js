mini.parse();
var grid = mini.get("datagrid_serviceModelDetail");
grid.load();
function showEdit(action,id){
	mini.open({
		url : ctx+"/page/msds/km/serviceModelDetail/serviceModelDetail_edit.jsp",
		title : "服务车型明细表",
		width : 600,
		height : 325,
		onload : function() {
	...
	if (rows.length > 0) {
		mini.confirm("确定删除选中记录？", "提示", function callback(msg) {
			if (msg == 'ok') {
				...
				$.ajax({
					url : ctx+"/serviceModelDetail/delete.action?id=" + id,
