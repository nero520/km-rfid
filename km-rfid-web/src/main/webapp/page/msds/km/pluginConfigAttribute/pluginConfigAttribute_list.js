mini.parse();
var grid = mini.get("datagrid_pluginConfigAttribute");
grid.load();
{
	}
function showEdit(action,id){
	mini.open({
		url : ctx+"/page/msds/km/pluginConfigAttribute/pluginConfigAttribute_edit.jsp",
		title : "支付插件明细表",
		width : 600,
		height : 150,
		onload : function() {
	...
	if (rows.length > 0) {
		mini.confirm("确定删除选中记录？", "提示", function callback(msg) {
			if (msg == 'ok') {
				...
				$.ajax({
					url : ctx+"/pluginConfigAttribute/delete.action?id=" + id,
